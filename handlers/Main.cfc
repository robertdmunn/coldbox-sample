<cfcomponent output="false" singleton extends="coldbox.system.EventHandler">
<cfproperty name="webSocketAppender" inject="logBox:logger:webSocketAppender"/>
...
<!--- In order for these events to fire, you must declare them in the ColdBox.cfc --->
	...
	<cffunction name="onException" returntype="void" output="false">
		...
		<cfscript>
			...
			var exceptionBean = Event.getValue("ExceptionBean");
			webSocketAppender.error("MESSAGE:#exceptionBean.getExceptionStruct().message#, DETAIL:#exceptionBean.getExceptionStruct().detail#",exceptionBean);
